...
OUTPUT_DIR="forensic_report_$(date +%Y%m%d_%H%M%S)" # Created a timestamped directory for forensic artifacts to prevent overwriting
API_KEY="${ANTHROPIC_API_KEY:?Error: ANTHROPIC_API_KEY environment variable is not set. Export it before running.}" # API key loaded securely from environment variable
...
mkdir -p "$OUTPUT_DIR" # Initialized output structure for data storage 
...
HEADER=$(head -n 1 "$TIMELINE") # Preserve header before sorting
BODY=$(tail -n +2 "$TIMELINE" | sort -t'|' -k5) # Sort only the data rows from oldest to newest
{ echo "$HEADER"; echo "$BODY"; } > "$TIMELINE" # Reassemble header + sorted body
...
AI_SUMMARY=$(DATA_SNIPPET="$DATA_SNIPPET" ANTHROPIC_API_KEY="$API_KEY" python3 - <<'PYEOF'
import os, json, urllib.request
# Safely build JSON payload using python3 to avoid shell injection from timeline data
data_snippet = os.environ.get("DATA_SNIPPET", "")
...
payload = {
    ...
    "messages": [
        {"role": "user", "content": f"Here is the timeline data:\n{data_snippet}"}
    ]
}
